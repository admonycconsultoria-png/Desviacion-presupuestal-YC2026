...
const [cfgPath, fuente, bal, ter, acc, nom, fe] = process.argv.slice(2);
const leer = (p) => (!p || p === "-" ? null : Exogena.leerLibro(XLSX, new Uint8Array(fs.readFileSync(p)), p));
const cfg = JSON.parse(fs.readFileSync(cfgPath, "utf8"));
const r = Exogena.ejecutar({ fuente, balance: leer(bal), terceros: leer(ter), accionistas: leer(acc), nomina: leer(nom),
  facturas: fe && fe !== "-" ? JSON.parse(fs.readFileSync(fe, "utf8")) : null }, cfg);
