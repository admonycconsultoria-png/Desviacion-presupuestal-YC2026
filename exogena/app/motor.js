...
(function (root) {
  ...
  // El ZIP puede traer otros ZIP (p. ej. una carpeta de Google Drive descargada)
  function leerFacturasArchivo(XLSX, datos, nombre, DOMParserClase, nivel = 0) {
    const u8 = datos instanceof Uint8Array ? datos : new Uint8Array(datos);
    const aTexto = (b) => new TextDecoder("utf-8").decode(b);
    if (/\.zip$/i.test(nombre) && nivel <= 3) {
      let cfb;
      try { cfb = XLSX.CFB.read(u8, { type: "array" }); } catch (e) { return []; }
      return cfb.FileIndex.filter((e) => e.type === 2 && /\.(xml|zip)$/i.test(e.name) && e.content)
        .flatMap((e) => leerFacturasArchivo(XLSX, e.content instanceof Uint8Array ? e.content : new Uint8Array(e.content), e.name, DOMParserClase, nivel + 1));
    }
